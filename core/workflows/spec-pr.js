...
const fin = await agent(
  [
    ...
    dryRun ? `` : `2. Existing PR? gh pr view "${branch}" --json url,state 2>/dev/null. If OPEN → prExisted=true, reuse its url (the push updated it). Else gh pr create --base "${base}" --head "${branch}" --title "spec(${change}): ${pre.title}" --body <body>. Body: the proposal's what-and-why; a "## Spec contract" section listing the touched capabilities (${specCanonicalPaths.join(', ') || 'see openspec/specs/'}); a "## Review checklist" (requirements use SHALL/MUST + ≥1 scenario; minimal/testable/consistent); a note "Merging this PR locks the contract; implementation follows via /opsx:ship ${change}"; and a final "🤖 Generated with [Claude Code](https://claude.com/claude-code)". Capture prUrl.`,
    dryRun ? `` : `3. LIFECYCLE (best-effort — NEVER fail the spec PR on this): extract the PR number from prUrl, then run \`node .claude/workflows/lib/lifecycle.js after-spec-pr-opened --change "${change}" --branch "${branch}" --spec-pr "<prUrl>" --spec-pr-number "<prNumber>"\`. It comments the linked ticket + records the spec PR ref; if the change isn't linked to a ticket it no-ops; if it errors, log and CONTINUE.`,
