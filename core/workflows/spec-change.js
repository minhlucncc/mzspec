...
// ---------------------------------------------------------------- Phase 1b: before-spec hook (best-effort, skipped on dryRun)
// Fire the before-spec agent-form hook so a board/issue is updated when spec authoring starts.
// The issue↔change binding lives in openspec/changes/<change>/github.json (written by
// /opsx:propose-gh in the task-github extension), NOT here — core stays task-source-agnostic, and
// the before-spec lifecycle call in Preflight no-ops when the change isn't linked.
if (!dryRun) {
  phase('Hooks')
