...
| **Templates** (`templates`) | optional, **skill-like planning guides** consulted during planning — `/opsx:template-create\|update\|remove\|list`. A template is a `<name>/TEMPLATE.md` (name + description + guide body); thin per-task templates **chain** into composites by prose the AI follows (a full `presale` chain, a selective `change-request` chain) — no resolver. If none matches a change, planning just proceeds normally | `openspec/templates/`, `.claude/commands/opsx/template-*.md`, `.claude/mzspec-templates/` |
