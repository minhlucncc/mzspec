 ...
 *
 * Unlike the original (which hardcoded the package inventory), this version is
 * **config-driven**: the toolchain dirs, gate commands, meta prefixes, bench
 * ladder, migration gate and custom project gates all come from the project's
 * `mzspec.config.json` (see lib/load-config.js). The public API is unchanged.
 *
 ...
 * omitted it is loaded from the nearest mzspec.config.json.
 */
...
function classify(file, config = loadConfig()) {
...
function gatesFor(c, config) {
  ...
  if (!tc) return [];
  return (tc.gates || []).map((g) => ({ name: g.name, cmd: subst(g.cmd, { dir: c.unitDir }) }));
}

function resolve(files, config = loadConfig()) {
  const bench = benchCfg(config);
  ...
  return { toolchains, units, flags, always, custom };
}
...
module.exports = { resolve, classify };
