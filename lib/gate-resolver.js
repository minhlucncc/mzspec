...
const { discover, findRepoRoot } = require('./discover.js');
...
// on stdin (one per line), and its stdout JSON IS the returned plan. Works for any
// language/framework (it's just an executable — sh, node, python, …).
function hookPath(startDir) {
  const root = findRepoRoot(startDir);
  return path.join(root, 'openspec', 'hooks', 'resolve-gates');
}
function isExecutable(p) {
  try { fs.accessSync(p, fs.constants.X_OK); return fs.statSync(p).isFile(); }
  catch { return false; }
}
function runHook(files, startDir) {
  const p = hookPath(startDir);
  if (!isExecutable(p)) return null;
  const out = execFileSync(p, [], { input: files.join('\n') + '\n', encoding: 'utf8', maxBuffer: 8 * 1024 * 1024 });
  const plan = JSON.parse(out);
  plan.source = 'hook';
  return plan;
}

// The full entrypoint: hook override first, else config/discovery resolution.
function resolvePlan(files, startDir) {
  const hooked = runHook(files, startDir);
