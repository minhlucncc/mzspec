 ...
 *
 * Mirrors load-config.js / gate-resolver.js: reads `taskSources` from the
 * project's mzspec.config.json, instantiates the selected adapter, and exposes a
 * uniform list/get/setStatus/comment surface. The /opsx:task workflow drives this
 * via the CLI (cli.js) the same way ship-code drives gate-resolver.js.
 */
...
function pickSourceEntry(config, name) {
  const sources = (config && config.taskSources) || [];
  if (!sources.length) throw new Error('no taskSources configured in mzspec.config.json');
  if (name) {
