...
class GhIssuesSource {
  ...
  async create({ title, body, labels } = {}) {
    if (!title || !String(title).trim()) throw new Error('gh-issues: create requires a title');
    const args = ['issue', 'create', ...this._repoArgs(), '--title', String(title), '--body-file', '-'];
    for (const l of labels || []) args.push('--label', String(l));
    // `gh issue create` prints the new issue URL on stdout.
    const out = run(this.bin, args, { input: String(body || '') }).trim();
    const url = (out.match(/https?:\/\/\S+\/issues\/\d+/) || [out.split('\n').pop()])[0] || '';
    const num = (url.match(/\/issues\/(\d+)/) || [])[1] || '';
    return makeTask({ id: num, title, body: body || '', status: 'todo', labels: labels || [], url, source: this.name });
  }
