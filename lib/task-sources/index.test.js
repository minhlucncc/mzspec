...
test('local-folder: round-trip list -> setStatus -> comment', async () => {
  ...
  // setAssignee writes/clears frontmatter, preserving the body
  await src.setAssignee('T-001-add-login', 'alice');
  const fmA = parseFrontmatter(fs.readFileSync(path.join(taskDir, 'task.md'), 'utf8'));
  assert.strictEqual(fmA.data.assignee, 'alice');
  assert.match(fmA.body, /log in/);
  await src.setAssignee('T-001-add-login', '');
  const fmB = parseFrontmatter(fs.readFileSync(path.join(taskDir, 'task.md'), 'utf8'));
  assert.ok(!('assignee' in fmB.data));
...
test('resolver: infers a gh-issues source when none configured (zero-config)', () => {
  // With no taskSources, fall back to inference from `git remote origin`. This test
  // repo has a github origin, so a gh-issues source is synthesized rather than thrown.
  const entry = pickSourceEntry({ taskSources: [] });
  assert.strictEqual(entry.type, 'gh-issues');
  assert.ok(entry.config && entry.config.repo);
});

test('resolver: clear error on unknown source type', () => {
  assert.throws(
...
test('gh-issues: parses list + builds expected argv for setStatus/comment', async () => {
  ...
  await src.setAssignee('42', '@me'); // -> issue edit --add-assignee @me
  process.env.VIEW_JSON = JSON.stringify({ assignees: [{ login: 'alice' }] });
  await src.setAssignee('42', ''); // clear -> reads assignees, issue edit --remove-assignee alice
  ...
  // assignee add + clear
  assert.ok(lines.some((a) => a[0] === 'issue' && a[1] === 'edit' && a.includes('--add-assignee') && a.includes('@me')));
  assert.ok(lines.some((a) => a[0] === 'issue' && a[1] === 'edit' && a.includes('--remove-assignee') && a.includes('alice')));
  ...
  delete process.env.CAPTURE_FILE; delete process.env.LIST_JSON; delete process.env.CREATE_OUT; delete process.env.VIEW_JSON;
});
...
test('mello: parses list + builds move/comment argv', async () => {
  ...
  await src.setAssignee('M-1', 'bob'); // -> ticket assign --user bob
  const lines = fs.readFileSync(capture, 'utf8').trim().split('\n').map((l) => JSON.parse(l));
  assert.ok(lines.some((a) => a[0] === 'ticket' && a[1] === 'move' && a.includes('In Progress')));
  assert.ok(lines.some((a) => a[0] === 'comment' && a[1] === 'add'));
  assert.ok(lines.some((a) => a[0] === 'ticket' && a[1] === 'assign' && a.includes('--user') && a.includes('bob')));
