...
## Composite templates: chaining

Keep per-task templates thin and **compose them** with a chain template — no duplication,
...
/ `proposal` / `translate` templates, a `presale` chain over all of them, and a
`change-request` chain that re-runs only what a change touches — all expressed as
markdown the AI follows.
