...
```mermaid
flowchart TB
    subgraph Hub["mework Hub (server)"]
        Catalog["Agent Catalog"]
        Bus["Bus (topics)"]
        Manager["Session Manager"]
    end

    Hub -->|dispatch / session.input| Daemon

    subgraph Daemon["mework Daemon (developer machine)"]
        direction TB
        Orchestrator["Orchestrator<br/>(Claude Code)"]
        MCP["mework-mcp<br/>(MCP server)"]
        Orchestrator <-->|stdio / MCP| MCP

        subgraph Sandboxes["Child sandboxes (spawned by MCP)"]
