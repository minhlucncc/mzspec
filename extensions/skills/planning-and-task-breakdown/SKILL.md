...
In this repo (Mezon Mentor Bot / "MeKnow"), the plan lives in the OpenSpec change: architectural decisions go in `openspec/changes/cNNNN-<slug>/design.md`, and the task list goes in `tasks.md`. Both are generated by `/opsx:propose` and refined here; you tick `tasks.md` off during `/opsx:apply`.
...
- The plan is OpenSpec: decisions go in `openspec/changes/cNNNN-<slug>/design.md`, the
