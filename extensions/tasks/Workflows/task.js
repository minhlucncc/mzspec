...
if (action === 'pull') {
  ...
  const made = await agent(
    [
      ...
      `3. Create it: node .claude/workflows/lib/openspec.js new change "cNNNN-<slug>".`,
      `4. Seed proposal.md from the task: get the proposal artifact path (node .claude/workflows/lib/openspec.js instructions proposal --change "cNNNN-<slug>" --json) and write a proposal whose "## What/## Why" are grounded in the task title + body. Keep it faithful to the task text; do not invent scope.`,
