...
# Core pipeline machinery is always refreshed. Extensions managed via mzspec CLI.
log "re-vendoring core pipeline ..."
bash "$SRC/scripts/install.sh" --dest "$DEST" --force --no-openspec

if [ "$FORCE_SKILLS" -eq 1 ]; then
  log "re-vendoring agent-skills (--force-skills) ..."
  bash "$SRC/extensions/agent-skills/install.sh" --dest "$DEST" --force
else
